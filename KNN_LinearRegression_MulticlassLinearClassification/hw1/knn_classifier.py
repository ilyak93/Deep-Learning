...
class KNNClassifier(object):
    ...
    def train(self, dl_train: DataLoader):
        ...
        n_classes = len(set(y_train.numpy()))
        # ========================
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            sorted_row_indices = torch.argsort(dist_matrix[:, i])
            best_k_indices = sorted_row_indices[:self.k]
            best_k_labels = self.y_train[best_k_indices]
            y_pred[i] = torch.bincount(best_k_labels).argmax()
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    a= (torch.sum(x1**2, axis=1)).reshape(-1, 1)
    b= -2 * (x1.mm(x2.T))
    c= (torch.sum(x2**2, axis=1)).reshape(1, -1)
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = None
    # ====== YOUR CODE: ======
    accuracy = len(torch.where(y == y_pred)[0]) / len(y_pred)
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for j in range(num_folds):
            dl_train, dl_valid = create_train_validation_loaders(dataset=ds_train, validation_ratio=1/num_folds)

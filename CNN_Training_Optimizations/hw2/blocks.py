...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.normal(torch.tensor(0.0), torch.tensor(wstd), (in_features, out_features))
        self.b = torch.normal(torch.tensor(0.0), torch.tensor(wstd), (out_features,))
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        self.db += torch.matmul(torch.ones(x.shape[0]), dout)
...
class Sigmoid(Block):
    ...
    def backward(self, dout):
        ...
        # dx = torch.matmul(torch.matmul(sig, 1-sig.T), dout)
        dx = sig * (1-sig) * dout
        # ========================
...
class CrossEntropyLoss(Block):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        x_y = x[range(N), y]
        log_sigma = torch.log(torch.sum(torch.exp(x), dim=1))

        loss = (-x_y + log_sigma).mean()
...
class Dropout(Block):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode and (self.p != 0):
            rand_mat = torch.zeros_like(x).uniform_()
            rand_mat[rand_mat > self.p] = 1
            rand_mat[rand_mat <= self.p] = 0

            self.grad_cache['rand_mat'] = rand_mat
            out = (x * rand_mat)
    ...
    def backward(self, dout):
        ...
        if self.training_mode:
            rand_mat = self.grad_cache['rand_mat']
            dx = dout * rand_mat
...
class MLP(Block):
    ...
    def __init__(self, in_features, num_classes, hidden_features=(),
        ...
        # ====== YOUR CODE: ======
        non_linearity = ReLU if activation == 'relu' else Sigmoid

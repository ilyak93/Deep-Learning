...
def run_experiment(run_name, out_dir='./results', seed=None, device=None,
    ...
    fit_res = None
    # ====== YOUR CODE: ======
    in_size = ds_train.data.shape[3], ds_train.data.shape[1], ds_train.data.shape[2]
    num_classes = len(ds_train.classes)

    model = model_cls(in_size=in_size, out_classes=num_classes,
                      channels=[conv_filter for conv_filter in filters_per_layer for _ in range(layers_per_block)], pool_every=pool_every,
                      hidden_dims=hidden_dims)
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(params=model.parameters(), lr=lr, weight_decay=1e-5)
    trainer = training.TorchTrainer(model, loss_fn, optimizer, device)
    
    bs_test = max([bs_train // 5, 1])

    train_dl = DataLoader(ds_train, batch_size=bs_train, shuffle=True)
    test_dl = DataLoader(ds_test, batch_size=bs_test, shuffle=True)

    fit_res = trainer.fit(train_dl, test_dl, num_epochs=epochs, checkpoints=checkpoints, early_stopping=5, print_every=1)
    # ========================

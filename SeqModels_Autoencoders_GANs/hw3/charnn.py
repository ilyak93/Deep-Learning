...
def char_maps(text: str):
    ...
    # ====== YOUR CODE: ======
    chars_list = sorted(set(text))
    char_to_idx = {char: idx for char, idx in zip(chars_list, range(len(chars_list)))}
    idx_to_char = {idx: char for char, idx in zip(chars_list, range(len(chars_list)))}
    # ========================
...
def remove_chars(text: str, chars_to_remove):
    ...
    # ====== YOUR CODE: ======
    new_text = ''.join([char for char in list(text) if char not in chars_to_remove])
    n_removed = len(text) - len(new_text)
    text_clean = new_text
    # ========================
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    #out_text=''
    with torch.no_grad():
        prev_sequence = torch.unsqueeze(chars_to_onehot(start_sequence, char_to_idx), dim=0).to(device=device, dtype=torch.float)
        hidden_state = None
        while len(out_text) < n_chars:
            output, hidden_state = model(prev_sequence, hidden_state)
            prob_output = hot_softmax(output[0, -1, :], dim=0, temperature=T)
            generated_char_idx = torch.multinomial(prob_output, 1)[0].item()
            generated_char = idx_to_char[generated_char_idx]
            out_text += generated_char
            prev_sequence = torch.unsqueeze(chars_to_onehot(generated_char, char_to_idx), dim=0).to(device=device, dtype=torch.float)
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        M = len(self.dataset) - (len(self.dataset) % step)
        idx_lists = [list(range(i, M, step)) for i in range(0, step)]
        idx = torch.Tensor(functools.reduce(operator.iconcat, idx_lists, [])).type(torch.int64)
        # ========================
        return iter(idx)
